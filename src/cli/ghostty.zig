...
const restore_session = @import("restore_session.zig");
...
pub const Action = enum {
    ...
    // Restore a saved session in the running Ghostty instance.
    @"restore-session",
    ...
    fn runMain(self: Action, alloc: Allocator) !u8 {
        return switch (self) {
            ...
            .@"restore-session" => try restore_session.run(alloc),
        };
    }
    ...
    pub fn options(comptime self: Action) type {
        comptime {
            return switch (self) {
                ...
                .@"restore-session" => restore_session.Options,
            };
        }
    }
};
